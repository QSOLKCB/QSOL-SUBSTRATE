...
def _compile_patterns(items: list[dict[str, str]], label: str) -> list[tuple[str, re.Pattern[str]]]:
    compiled = []
    for item in items:
        if set(item) != {"id", "regex"}:
        ...
        if isinstance(node, dict):
            for key, item in node.items():
                if str(key).lower() in forbidden_field_names:
                    raise ExportError(f"forbidden field name {key!r} encountered in {label}{path}")
            ...
            for pattern_id, regex in secret_patterns:
                if regex.search(node):
                    raise ExportError(f"secret pattern {pattern_id!r} detected in {label}{path}")
            for pattern_id, regex in private_patterns:
                if regex.search(node):
                    raise ExportError(f"private-reference pattern {pattern_id!r} detected in {label}{path}")
    walk(value, "")
...
def _validate_source_root(source_root: Path, policy: dict[str, Any]) -> dict[str, Any]:
    ...
    manifest_rel = _safe_relative(source_contract.get("manifest_path", ""), "source manifest path")
    ...
    if actual != expected:
        raise ExportError(f"source protocol mismatch: expected {expected!r}, got {actual!r}")
    return manifest
...
def _load_payload(substrate_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(payload_files, list) or not payload_files or not all(isinstance(p, str) for p in payload_files):
...
def _find_source_record(source_doc: Any, source_spec: dict[str, Any]) -> tuple[Any, str]:
    if "pointer" in source_spec:
        if "collection_pointer" in source_spec or "match" in source_spec:
            raise ExportError("source spec may use pointer or collection_pointer+match, not both")
...
def _validate_field_rule(rule: dict[str, Any], entry_id: str) -> None:
    ...
    if len(modes) != 1:
        raise ExportError(f"{entry_id}: field rule requires exactly one of from, value, redact_from")
...
def _build_record(
    *,
    ...
    if not all(isinstance(v, str) and v for v in (record_id, record_type, epistemic_state)):
        raise ExportError(f"{entry_id}: record id/type/epistemic_state must be non-empty strings")

    fields = record_spec["fields"]
    if not isinstance(fields, list):
        raise ExportError(f"{entry_id}: record.fields must be an array")

    reserved_targets = {"/id", "/record_type", "/visibility", "/epistemic_state", "/source_refs"}
    output: dict[str, Any] = {
        "id": record_id,
...
def _apply_record(payload: dict[str, Any], entry: dict[str, Any], record: dict[str, Any]) -> None:
    ...
    if not isinstance(target_path, str) or not isinstance(collection_pointer, str) or not isinstance(allow_create, bool):
        raise ExportError(f"{entry['id']}: target requires path, collection_pointer, and boolean allow_create")
    if target_path not in payload:
        raise ExportError(f"{entry['id']}: target path is not a canonical payload file: {target_path}")

    collection = _pointer_get(payload[target_path], collection_pointer)
    if not isinstance(collection, list):
        raise ExportError(f"{entry['id']}: target collection pointer must resolve to an array")

    record_id = record["id"]
    indexes = [idx for idx, item in enumerate(collection) if isinstance(item, dict) and item.get("id") == record_id]
    if len(indexes) > 1:
        raise ExportError(f"{entry['id']}: target contains duplicate record id {record_id!r}")
    if indexes:
        collection[indexes[0]] = record
    elif allow_create:
        collection.append(record)
    else:
        raise ExportError(f"{entry['id']}: target record {record_id!r} does not exist and allow_create=false")
    ...
    if sort_by is not None:
        ...
        try:
            ...
            raise ExportError(f"{entry['id']}: target.sort_by values are not mutually sortable") from exc


def _canonical_config_fingerprint(policy: dict[str, Any], include: dict[str, Any], exclude: dict[str, Any]) -> str:
    return _sha256_bytes(_json_bytes({"policy": policy, "include": include, "exclude": exclude}))
...
def run_export(
    *,
    ...
    if _within(substrate_root, output_root) and not allow_output_inside_repo:
        raise ExportError("refusing to write export output inside the public repository without --allow-output-inside-repo")
    if audit_manifest is not None:
        audit_manifest = audit_manifest.resolve()
        if _within(output_root, audit_manifest):
            raise ExportError("private audit manifest may not be written inside the public export output")
    ...
    immutable = set(policy.get("immutable_payload_files", []))
    source_path_globs = exclude.get("forbidden_source_path_globs", [])
    forbidden_field_names = {str(x).lower() for x in exclude.get("forbidden_field_names", [])}
    secret_patterns = _compile_patterns(exclude.get("secret_patterns", []), "secret")
    private_patterns = _compile_patterns(exclude.get("private_reference_patterns", []), "private-reference")
    ...
    temp_root = Path(tempfile.mkdtemp(prefix=f".{output_root.name}.", dir=str(output_root.parent)))
    applied: list[str] = []
    directive_audit: list[dict[str, Any]] = []
    total_redactions = 0
    try:
        ...
        for entry in entries:
            ...
            if entry.get("enabled") is not True:
                continue
            if entry.get("visibility") != "public":
                raise ExportError(f"{entry_id}: enabled directive requires visibility='public'")
            ...
            if _forbidden_source_path(source_rel_text, source_path_globs):
                raise ExportError(f"{entry_id}: source path is forbidden by policy: {source_rel_text}")
            ...
            if source_path.suffix != ".json":
                raise ExportError(f"{entry_id}: Phase 2 directives currently require JSON source files")
            ...
            if target_path in immutable:
                raise ExportError(f"{entry_id}: target file is immutable under private export policy: {target_path}")
            if target_path not in payload:
                raise ExportError(f"{entry_id}: target is not a canonical payload file: {target_path}")
            ...
            _write_canonical(temp_root / target_path, payload[target_path], _kind_for_path(target_path))
        ...
        for rel in payload_files:
            path = temp_root / rel
            parsed = _load_json(path) if _kind_for_path(rel) == "json" else _load_jsonl(path)
            _scan_value(
                parsed,
                secret_patterns=secret_patterns,
                private_patterns=private_patterns,
        ...
        source_protocol = _pointer_get(source_manifest, policy["source_contract"]["protocol_pointer"])
        ...
        manifest_rel = _safe_relative(policy.get("output_manifest", "export-manifest.json"), "output manifest")
        ...
        if output_root.exists():
            if not force:
                raise ExportError(f"output already exists; use --force to replace: {output_root}")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source-root", required=True, type=Path, help="Local checkout/root of private QSOL-CONTEXT.")
    parser.add_argument("--substrate-root", type=Path, default=repo_root, help="QSOL-SUBSTRATE repository root.")
    parser.add_argument("--output", required=True, type=Path, help="Destination directory for the reviewable public bundle.")
    parser.add_argument("--policy", type=Path, default=repo_root / "public_export/policy.json")
    parser.add_argument("--include", type=Path, default=repo_root / "public_export/include.json")
    parser.add_argument("--exclude", type=Path, default=repo_root / "public_export/exclude.json")
    parser.add_argument("--audit-manifest", type=Path, help="Optional PRIVATE audit manifest path; must be outside public output.")
    parser.add_argument("--allow-output-inside-repo", action="store_true", help="Explicitly permit output under the public repo.")
